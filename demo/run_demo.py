...
SCRIPT = [
    {
        ...
        "note": ("The spec's version of this slide claims retrieval surfaces a "
                 ...
                 "49th. The reranked top-10 is 100% literal-HER2. "
                 "AND THE TOKEN IS CAUSAL, not correlated: take the rank-1 document, "
                 "'Antigen-binding constructs targeting HER2', and rename HER2 to "
                 "ErbB2 -- the same protein, every other byte identical, still 331 "
                 "tokens of on-topic antibody engineering. It falls 5.47 logits, "
                 "rank 1 -> 38. THE CLAIM, AND ONLY THIS CLAIM: the reranker carries "
                 "a lexical dependence large enough, on this query, to bury a "
                 "semantically identical document 37 places. It is NOT a keyword "
                 "filter -- the 45 literal docs span ~100x in score, so it "
                 "discriminates hard among them -- and the effect is not a constant "
                 "bonus: adding HER2 to the ErbB2 patent buys only +1.76 logits, to "
                 "rank 38, not into the top 10. So surface form is a large part of "
                 "what buries the bridge, and not all of it. "
