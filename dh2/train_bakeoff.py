...
def is_fn_flagged(fn_flag: dict | None, query_id: str, doc_id: str) -> bool:
    """BUG #7 FIX: the flag is per-(QUERY, DOC), not per-doc.

    p1_1 wrote `fn_flag[document_id] = True`, so a document flagged as a suspected false
    negative for ONE query was banned as a negative for EVERY query -- 38,861 documents,
    globally. "Scores as high as the positive" is a statement about a (query, document)
    pair; it says nothing about that document's relationship to 2,999 other queries.

    Accepts both formats:
      new:    {query_id: {doc_id: true}}  or  {query_id: [doc_id, ...]}
      legacy: {doc_id: true}              -> honored, with a warning, so an old
                                             fn_flag_v1.json still runs (arm B's whole
                                             point is the comparison, and silently
                                             changing its semantics mid-experiment would
                                             be worse than the bug).
    """
    if not fn_flag:
        return False
    entry = fn_flag.get(query_id)
    if entry is None:
        # legacy flat format: {doc_id: true}
        val = fn_flag.get(doc_id)
        return bool(val) if isinstance(val, bool) else False
    if isinstance(entry, dict):
        return bool(entry.get(doc_id, False))
    if isinstance(entry, (list, set, tuple)):
        return doc_id in entry
    return False


def fn_flag_is_legacy(fn_flag: dict | None) -> bool:
    """True if the sidecar is the old global {doc_id: bool} shape."""
    if not fn_flag:
        return False
    return all(isinstance(v, bool) for v in fn_flag.values())


def make_triples_for_marginmse(pq: PackedQuery, n_neg: int = 4,
                               hard_filter: bool = False,
                               fn_flag: dict | None = None,
    ...
    if hard_filter and fn_flag:
        neg_idx = [i for i in neg_idx
                   if not is_fn_flagged(fn_flag, pq.query_id, pq.doc_ids[i])]
...
def pack_listwise(pq: PackedQuery, max_list: int = 32, seed: int = 0,
                  stratify: bool = True) -> dict:
    """Pack a query's candidates into a max_list window for listwise/LSEPair training.

    BUG #6 FIX: this used to be `pq.doc_ids[:max_list]` -- the first max_list candidates
    in POOL ORDER, unshuffled, with no positive/negative guarantee. Measured: 32.7% of
    queries had ZERO positives in the window (so D and E trained on a listwise objective
    with nothing to rank up) and 3.3% had 8/8 positives (nothing to rank down). Arms
    A/B/C got shuffled triples; D/E/F got whatever pooling happened to emit first. That
    is not an objective comparison, it is a data-order comparison. With the denser labels
    (36% of queries now have >=5 positives) the imbalance gets worse, not better.

    Stratified packing guarantees both classes are represented when both exist: reserve
    up to half the window for positives, fill the rest with negatives, backfill from
    whichever class remains if one runs short, then shuffle so position carries no
    signal. Deterministic given `seed`.

    Returns the same dict contract as before (padded to max_list), plus "n_positive".
    """
    if not stratify:
        ...
        L = len(d)
        return {"doc_ids": d + [""] * (max_list - L),
                "teacher_rel": rel + [0.0] * (max_list - L),
                "is_positive": [int(p) for p in pos] + [0] * (max_list - L),
                "mask": [1] * L + [0] * (max_list - L),
                "n_positive": int(sum(pos))}

    rng = np.random.default_rng(seed)
    pos_idx = [i for i, p in enumerate(pq.is_positive) if p]
    neg_idx = [i for i, p in enumerate(pq.is_positive) if not p]

    want_pos = min(len(pos_idx), max(1, max_list // 2)) if pos_idx else 0
    want_neg = min(len(neg_idx), max_list - want_pos)
    # if one class is short, give its slots to the other rather than padding
    if want_pos + want_neg < max_list:
        spare = max_list - want_pos - want_neg
        extra_pos = min(spare, len(pos_idx) - want_pos)
        want_pos += extra_pos
        spare -= extra_pos
        want_neg += min(spare, len(neg_idx) - want_neg)

    take = []
    if want_pos:
        take += [int(i) for i in rng.choice(pos_idx, size=want_pos, replace=False)]
    if want_neg:
        take += [int(i) for i in rng.choice(neg_idx, size=want_neg, replace=False)]
    rng.shuffle(take)

    d = [pq.doc_ids[i] for i in take]
    rel = [pq.teacher_rel[i] for i in take]
    pos = [pq.is_positive[i] for i in take]
    L = len(d)
    pad = max_list - L
    return {
        "doc_ids": d + [""] * pad,
...
def train_arm(arm_name: str, arm_spec: dict, labels_path: str, base_model: str,
    ...
    if arm_spec.get("hard_filter") and fn_flag_is_legacy(fn_flag):
        print("[train_arm] WARNING: fn_flag sidecar is the LEGACY global {doc_id: bool} "
              "shape (bug #7) -- a doc flagged for one query is banned as a negative for "
              "ALL queries. Re-run p1_1 to emit the per-query sidecar.", flush=True)
        ...
        for qi, qid in enumerate(qids):
            ...
            if kind in ("mnrl", "marginmse"):
                ...
                if kind == "marginmse":
                    ...
                    # BUG #4: the scale is finally applied, and WHICH side it scales is
                    # an explicit, ablatable choice (cfg.marginmse_mode).
                    loss = losses["MarginMSE"](scale=cfg.marginmse_scale,
                                               mode=cfg.marginmse_mode)(
                        s_pos, s_neg, t_pos, t_neg)
                ...
                # BUG #6: seeded, stratified window (was: first step_cap in pool order,
                # which left 32.7% of queries with zero positives to rank up).
                packed = pack_listwise(pq, max_list=step_cap, seed=qi)
                # BUG #3: skip zero-positive queries for lsepair, matching rand1lh's
                # guard. Stratification makes this rare, but a query whose labels contain
                # no positive at all still reaches here.
                if kind in ("lsepair", "rand1lh") and packed["n_positive"] == 0:
                    continue
                ...
                if kind == "listwise":
                    ...
                    # BUG #5: teacher enters the softmax in the student's units.
                    loss = losses["ListwiseKL"](
                        T_student=cfg.listwise_T_student,
                        T_teacher=cfg.listwise_T_teacher,
                        aux_weight=cfg.contrastive_aux_weight,
                        scale=cfg.marginmse_scale,
                        scale_teacher=cfg.listwise_scale_teacher)(s, tr, mask)
                elif kind == "lsepair":
                    isp = torch.tensor([packed["is_positive"]], device=s.device)
                    loss = losses["LSEPair"](scale=cfg.marginmse_scale)(s, isp, mask)
