...
def main() -> int:
    ...
    # --- B0 DIAGNOSTIC (side output only; scoring path untouched) ---
    import collections as _c
    _pools = C.REPORTS2_DIR / f"pools_{args.name}.jsonl"
    with open(_pools, "w") as _fh:
        for _q, _ids in cache.items():
            _fh.write(json.dumps({"query": _q, "pool": _ids,
                                  "sources": [doc_source.get(_d, "unknown") for _d in _ids]}) + "\n")
    _t10 = _c.Counter(); _p100 = _c.Counter(); _anyoa = 0
    for _q, _ids in cache.items():
        _t10.update(doc_source.get(_d, "unknown") for _d in _ids[:10])
        _p100.update(doc_source.get(_d, "unknown") for _d in _ids[:100])
        if any(doc_source.get(_d, "unknown") == "openalex" for _d in _ids[:10]): _anyoa += 1
    _b0 = {"n_queries": len(cache), "top10": dict(_t10), "pool100": dict(_p100),
           "queries_with_openalex_in_top10": _anyoa,
           "top10_slots_total": sum(_t10.values()), "pool100_slots_total": sum(_p100.values())}
    print("[B0] " + json.dumps(_b0))
    (C.REPORTS2_DIR / f"b0_{args.name}.json").write_text(json.dumps(_b0, indent=2))
    # --- end B0 ---
