...
if [[ -f "$SCREEN_JSON" ]]; then
  "$PY" - "$SCREEN_JSON" <<'PYIN' > "$LOGS/screen_eval.log" 2>&1
import json,sys,subprocess,os
from pathlib import Path
sys.path.insert(0, os.environ["PWD"])
models=json.loads(Path(sys.argv[1]).read_text())
root=os.environ["ROOT"]; py=os.environ.get("PYTHON","/venv/main/bin/python")
best=None
for name,mdir in models.items():
    ...
    rep=Path(os.environ["DH2_ROOT"])/"reports"/f"eval_screen_{name}.json"
    if rep.exists():
...
Path(os.environ["DH2_ROOT"],"bakeoff","winner.json").write_text(json.dumps({"winner":best[0] if best else None,"r10":best[1] if best else 0}))
print("WINNER:", best)
PYIN
  ROOT="$ROOT" PYTHON="$PY" DH2_ROOT="$DH2_ROOT" PWD="$ROOT" true
fi
